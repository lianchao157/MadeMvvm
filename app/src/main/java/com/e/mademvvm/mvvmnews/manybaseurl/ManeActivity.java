...
public class ManeActivity extends AppCompatActivity {
            ...
            public void onClick(View v) {
                ...
                if (httpUrl == null || !httpUrl.toString().equals(mUrl1.getText().toString())) { //可以在 App 运行时随意切换某个接口的 BaseUrl
                 ...
                 NetWorkManager
                        ...
                        .getOneApiService()
                        .getUsers(1, 10)
                        .compose(ManeActivity.this.<ResponseBody>getDefaultTransformer())
    ...
    /**
     * 什么是高级模式? 什么是 pathSegment? 建议先看看最上面的类注释! {@link ManeActivity}
     */
    public void startAdvancedModel(View view) {
        final EditText editText = new EditText(ManeActivity.this);
    ...
    private class ChangeListener implements onUrlChangeListener {
        ...
        public void onUrlChangeBefore(HttpUrl oldUrl, String domainName) {
            Log.d("ManeActivity", String.format("The oldUrl is <%s>, ready fetch <%s> from DomainNameHub",
                    oldUrl.toString(),
                    domainName));
        }
